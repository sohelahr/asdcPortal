            ...
            <div class="card">
                ...
                @if(\App\Http\Helpers\CheckPermission::hasPermission('create.occupations'))
                    <div class="d-flex p-1 m-0 border header-buttons">
                        <div>
                            <button class="btn bg-white" type="button" data-bs-toggle="modal" data-bs-target="#occupation-create">
                ...
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        <table class="display datatables" id="occupations">
                        ...
                        <form method="POST" action="{{url('occupation/create')}}" id="myForm">
                            ...
                            <div class="modal-body">
                                <div class="form-group">
                                    ...
                                    <input id="name" class="form-control" type="text" name="name" placeholder="eg: Software Developer">
                                </div>
                            </div>
                    ...
                    <div class="modal-content">
                        <form method="POST" action="{{url('occupation/edit')}}" id="edit-form">
                            ...
                            <div class="modal-body">
                                <div class="form-group">
                                    <label for="name">Name <sup class="text-danger">*</sup></label>
                                    <input id="edit-name" class="form-control" type="text" name="name">
                                </div>
                            </div>
                            <div class="modal-footer">
                                <input type="submit" value="Submit" class="btn btn-primary">  
...
@section('jcontent')
    <script>
        ...
        function deleteOccupationConfirm(id){
            swal({
                title: "Warning",
        ...
        @if(\Illuminate\Support\Facades\Session::has('created'))    
            Notify('Created','Occupation Was Successfully Created','success')
        @elseif(\Illuminate\Support\Facades\Session::has('updated'))
            Notify('Updated','Occupation Was Successfully Update','info')
        @elseif(\Illuminate\Support\Facades\Session::has('deleted'))
            Notify('Deleted','Occupation Was Successfully Deleted','warning')
        @elseif(\Illuminate\Support\Facades\Session::has('error'))
            Notify('Danger','Something Went Wrong','danger')  
        @elseif(\Illuminate\Support\Facades\Session::has('prohibited'))
            Notify('Cannot Delete','This occupation already has admissions','warning')
        @endif
                    ...
                    {data: 'perm',render:function(data,type,row){
                        return `<div>
                                    <div  class="d-flex p-0 m-0">
                                        ${data.edit_perm ? 
                                        `<button class="form-btn form-btn-warning ms-4 me-2" onclick="Editoccupation(${row.id})">
                                        ...
                                        `<div class="ml-2">
                                                    <button type=submit class="form-btn form-btn-danger ms-2" onclick="deleteOccupationConfirm(${row.id})">
                                                        <i class="fa fa-trash"></i>
                                                    </button>
                                            </div>        
                                                <form action='occupation/delete/${row.id}' id='delete_form_${row.id}' method="post" class="d-none">
                                                    @csrf
                                                </form>
                                        `: null}
                                    </div>
